...
- [x] #74 merged → tag v0.19.1 (`a80741d2`) → npm latest ✓ (registada no fecho 0.19.1)
...
- [x] #76 merged (após fixes CodeQL) → tag v0.19.3 (`12c5188c`) → release.yml success → npm latest 0.19.3 (gitHead confirmado)


## Update — ciclo 0.19.4 (2026-09-04, «a promessa do minimal»)
- [x] Tecto por-id por detail (min 78/std 81/ultra 86 = floor((promessa−base)/declive); full sem tecto — oráculo protegido); needs_decomposition declarado c/ requirement_ceiling + receita executável (lote = só task+risk_level+concerns; concerns SOMAM, não restringem); custo projectado no next do select; payload-ceilings.ts + coerência; TC-F-34; eval 0 FAIL; ouro 10/10
- [ ] PR → merge → tag v0.19.4 → npm latest
